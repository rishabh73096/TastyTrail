...
const CardItem = ({ id, img, price, qty, name, handletost }) => {
  ...
  return (
    <div className="relative flex items-center justify-between bg-white p-2 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 ease-in-out m-1">
      <img src={img} alt="no image" className="w-16 h-16 rounded-lg " />
      ...
      <MdDelete
        className="absolute top-2 right-2 text-2xl text-gray-600 cursor-pointer hover:text-red-500 transition-colors"
        onClick={() => {
          dispatch(removeToCart({ id, img, name, qty, price }));
          handletost(name);
        }}
      />

      <div className="flex items-center space-x-2 absolute top-10 right-1">
        {/* Disable decrement if qty is 1 */}
        <CiCircleMinus
        ...
        <CiCirclePlus
          className="text-2xl text-gray-600 cursor-pointer hover:text-green-500 transition-colors"
